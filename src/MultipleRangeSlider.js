...
import { DEFAULT_CONFIG, roundValue, valueToPercent, percentToValue } from './Utils.js';
...
export class MultipleRangeSlider extends HTMLElement {
    static observedAttributes = ['min', 'max', 'values', 'name', 'precision', 'step', 'disabled'];
        ...
        return roundedValues.map((value, index) => {
            ...
            return `
                <div class="thumb ${isDisabled}" 
                     ...
                     part="thumb">
                    <div class="value" part="value">${value.toFixed(this.precision)}</div>
                </div>
            `;
        }).join('');
    }
    ...
    updateRange() {
        ...
        const rightPercent = valueToPercent(sortedValues[sortedValues.length - 1], this.min, this.max);
    ...
    setThumbValue(index, value) {
        ...
        const thumb = this.shadowRoot.querySelector(`.thumb[data-index="${index}"]`);
        if (!thumb) return;
