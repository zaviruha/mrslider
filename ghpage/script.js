...
function getDemo1Values() {
  ...
  document.getElementById("globalLogOutput").textContent = `Значения: [${values
    .map((v) => v.toFixed(2))
    .join(", ")}]`;
}
...
function demo2Random() {
  ...
  document.getElementById(
    "globalLogOutput"
  ).textContent = `Значения: [${randomValues.join(", ")}]`;
}
...
function handleFormSubmit(event) {
  ...
  document.getElementById(
    "globalLogOutput"
  ).textContent = `Отправленные значения: ${values.join(", ")}`;
}
